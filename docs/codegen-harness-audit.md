...
**Update 2026-06-04 — render-error classification recovery (§2.13.1):** Recovery project shipped (spec: `docs/superpowers/specs/2026-06-04-render-error-classification-recovery.md`, plan: `docs/superpowers/plans/2026-06-04-render-error-classification-recovery.md`). Backfill script `packages/backend/scripts/backfill-render-errors.ts` re-classified 285 historical `render_status='error'` rows (15 in PCB Cases, 270 elsewhere); 149 were recovered to a real category (71 from `agentConversation` tool-results, 78 from non-lossy `render_error` strings), 136 fell through to UNKNOWN. After backfill plus runtime persistence patches, the 435 errored rows globally now bucket as: UNKNOWN 247 (57%), GEOMETRY 94, API_MISUSE 50, KERNEL_ERROR 33, TYPE_ERROR 10, INFRASTRUCTURE 1 — zero null categories. PCB Cases (15 rows) all classify as UNKNOWN, consistent with the agentConversation shape audit done during T6.5 (mostly legacy non-array conversations or last tool-result being a non-render tool). New errors going forward classify at write time via `extractAndClassifyLastRenderError()`, so the UNKNOWN bucket should shrink as the old rows churn out. Next lever for the residual UNKNOWN bucket is §13.2 (sub-classify GEOMETRY) and triage of the recovered raw messages now visible in the Render Errors admin tab.
