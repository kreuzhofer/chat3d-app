import { useCallback, useEffect, useRef, useState } from "react";
import { Layers } from "lucide-react";
import { type RecentModel, getRecentModelScreenshotUrl } from "../api/public.api";

interface RecentModelsCarouselProps {
  models: RecentModel[];
}

/** Visible card count per breakpoint (matches Tailwind md: = 768px). */
function getVisibleCount(): number {
  if (typeof window === "undefined") return 5;
  if (window.innerWidth >= 768) return 5;
  if (window.innerWidth >= 640) return 3;
  return 2;
}

export function RecentModelsCarousel({ models }: RecentModelsCarouselProps) {
  const [queue, setQueue] = useState(() => [...models]);
  const [sliding, setSliding] = useState(false);
  const [paused, setPaused] = useState(false);
  const [visibleCount, setVisibleCount] = useState(getVisibleCount);
  const timerRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  // Keep queue in sync if models prop changes
  useEffect(() => {
    setQueue([...models]);
  }, [models]);

  // Track responsive breakpoint
  useEffect(() => {
    const onResize = () => setVisibleCount(getVisibleCount());
    window.addEventListener("resize", onResize);
    return () => window.removeEventListener("resize", onResize);
  }, []);

  const rotate = useCallback(() => {
    setSliding(true);
    // After the CSS transition completes, reorder the queue and reset
    timerRef.current = setTimeout(() => {
      setQueue((prev) => [...prev.slice(1), prev[0]]);
      setSliding(false);
    }, 500);
  }, []);

  // Auto-rotation interval
  useEffect(() => {
    if (paused || queue.length <= visibleCount) return;
    const id = setInterval(rotate, 5000);
    return () => {
      clearInterval(id);
      if (timerRef.current) clearTimeout(timerRef.current);
    };
  }, [paused, rotate, queue.length, visibleCount]);

  if (models.length === 0) return null;

  // Card width as percentage: each card takes 1/visibleCount of the container
  // minus the gap (1rem = 16px between cards)
  const gapRem = 1;
  const cardCalc = `calc((100% - ${(visibleCount - 1) * gapRem}rem) / ${visibleCount})`;
  const shiftCalc = `calc((100% - ${(visibleCount - 1) * gapRem}rem) / ${visibleCount} + ${gapRem}rem)`;

  return (
    <section className="rounded-2xl bg-[linear-gradient(135deg,#0f172a_0%,#1e293b_40%,#134e4a_100%)] p-8 text-slate-100 md:p-12">
      {/* Section header */}
      <div className="mb-6 space-y-2">
        <p className="inline-flex items-center gap-2 rounded-full border border-white/20 px-3 py-1 text-xs uppercase tracking-[0.2em] text-emerald-200">
          <Layers className="h-3.5 w-3.5" />
          Model Library
        </p>
        <h2 className="text-xl font-semibold md:text-2xl">Recently added to the library</h2>
        <p className="text-sm text-slate-300">
          Approved models generated by our AI pipeline — browse the latest additions.
        </p>
      </div>

      {/* Carousel track */}
      <div
        className="overflow-hidden"
        onMouseEnter={() => setPaused(true)}
        onMouseLeave={() => setPaused(false)}
      >
        <div
          className="flex"
          style={{
            gap: `${gapRem}rem`,
            transform: sliding ? `translateX(calc(-1 * ${shiftCalc}))` : "translateX(0)",
            transition: sliding ? "transform 500ms ease-in-out" : "none",
          }}
        >
          {queue.map((model) => (
            <div
              key={model.id}
              className="shrink-0 rounded-xl border border-white/15 bg-black/20 p-3"
              style={{ width: cardCalc }}
            >
              <div className="aspect-square overflow-hidden rounded-lg bg-black/30">
                <img
                  src={getRecentModelScreenshotUrl(model.id)}
                  alt={model.promptText}
                  className="h-full w-full object-contain"
                  loading="lazy"
                  onError={(e) => {
                    (e.currentTarget.parentElement as HTMLElement).classList.add("hidden");
                  }}
                />
              </div>
              <p className="mt-2 truncate text-sm font-medium text-slate-100" title={model.promptText}>
                {model.promptText}
              </p>
              <p className="truncate text-xs text-emerald-200/70">{model.categoryName}</p>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
}
