...
import { confirmWaitlistEmail, joinWaitlist } from "../api/waitlist.api";
...
describe("waitlist api client", () => {
  ...
  it("confirms email by token", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith("/api/waitlist/confirm-email?token=tok-1", expect.any(Object));
  });
});
